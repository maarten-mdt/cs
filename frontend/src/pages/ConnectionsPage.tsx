...
const INTEGRATIONS: { id: string; name: string; description: string; keys: { key: string; label: string; secret?: boolean; placeholder?: string }[] }[] = [
  ...
  { id: "shopify_ca", name: "Shopify CA", description: "Canada store — OAuth2 client credentials or static token", keys: [{ key: "SHOPIFY_CA_DOMAIN", label: "Store domain", placeholder: "mdt-ca.myshopify.com" }, { key: "SHOPIFY_CA_CLIENT_ID", label: "Client ID", secret: true }, { key: "SHOPIFY_CA_CLIENT_SECRET", label: "Client Secret", secret: true }, { key: "SHOPIFY_CA_TOKEN", label: "Or static access token (if not using OAuth)", secret: true }] },
  { id: "shopify_us", name: "Shopify US", description: "US store — OAuth2 client credentials or static token", keys: [{ key: "SHOPIFY_US_DOMAIN", label: "Store domain", placeholder: "mdt-us.myshopify.com" }, { key: "SHOPIFY_US_CLIENT_ID", label: "Client ID", secret: true }, { key: "SHOPIFY_US_CLIENT_SECRET", label: "Client Secret", secret: true }, { key: "SHOPIFY_US_TOKEN", label: "Or static access token (if not using OAuth)", secret: true }] },
  { id: "shopify_int", name: "Shopify INT", description: "International store — OAuth2 client credentials or static token", keys: [{ key: "SHOPIFY_INT_DOMAIN", label: "Store domain", placeholder: "mdt-int.myshopify.com" }, { key: "SHOPIFY_INT_CLIENT_ID", label: "Client ID", secret: true }, { key: "SHOPIFY_INT_CLIENT_SECRET", label: "Client Secret", secret: true }, { key: "SHOPIFY_INT_TOKEN", label: "Or static access token (if not using OAuth)", secret: true }] },
